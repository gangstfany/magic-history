...
test('publishes exact immutable Unit 4 cards outside map records', () => {
  ...
  assert.ok(Object.isFrozen(api.getUnitCard('context')));
  assert.ok(Object.isFrozen(api.getUnitCard('synthesis')));
...
test('rejects malformed actor, term, evidence, source, and raw record structures', () => {
  const cases = [
    ...
    ['missing source id', 'delete RAW_RECORDS[0].source.id;', firstId, 'missing source id'],
    ['missing source locator', 'delete RAW_RECORDS[0].source.locator;', firstId, 'missing source locator'],
  ];
  for (const [label, injection, id, rule] of cases) {
