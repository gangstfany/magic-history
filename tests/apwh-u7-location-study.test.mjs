...
test('validates graph connections before publication and resists ordinary-shape attacks',()=>{
  ...
  assertInvalid(replace('Japan’s earlier regional victory strengthened military confidence and strategic interest in Manchuria, helping create conditions officers used to justify expansion in 1931.','This happened before the next event.'),/mechanism/);
...
test('keeps required Unit 7 map-anchor and atrocity-comparison caveats in the ledger',()=>{
  ...
  for(const id of records.filter(record=>record[1]==='25').map(record=>record[0]))assert.match(claimsById.get(id),/St\. Petersburg was named Petrograd in 1917/);
